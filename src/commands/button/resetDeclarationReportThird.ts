...
export const customId = 'reset_declaration_report_third';
...
export async function execute(interaction: ButtonInteraction) {
  ...
  await DataSource.getRepository(Declaration)
    ...
    .andWhere('declaration.attackCount = :attackCount', { attackCount: 3 })
    .execute();
  await DataSource.getRepository(Report)
    ...
    .andWhere('report.attackCount = :attackCount', { attackCount: 3 })
    .execute();

  await interaction.reply({
    content: '3凸目の宣言・報告をリセットしました。',
    ephemeral: true,
  });
}
